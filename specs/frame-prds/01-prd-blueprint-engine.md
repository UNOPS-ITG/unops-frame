...
**BP-27 (P2).** Overlay convergence. The engine reports, per catalog entry, where subscribing workspaces' overlays (BP-19) have independently converged — the same added field name, the same tightened property — with workspace counts, surfaced to the steward as fold-in candidates feeding the BP-17 working file and the next upstream version. Overlays are structured records, so convergence is a query, not telemetry archaeology. This is the catalog's growth loop: an overlay three workspaces wrote independently is the estate telling the steward what the base Blueprint is missing, and without the report that signal accrues silently per workspace — the reference implementation's equivalent overlay records accumulate per site for years precisely because nothing aggregates them upstream. The report is a suggestion surface only; folding a field in remains an ordinary BP-12 version with BP-17 review where required. Convergence reporting covers BP-28 extension fields identically: an extension field several workspaces independently add is a fold-in candidate for the upstream base.

**BP-28 (P2).** Workspace extensions — the locked base. *(Supersedes the added-fields overlay kind in BP-19; owner decision, August 2026.)* A subscribed organizational Blueprint is **locked**: its field set, child collections and naming are never modified by a subscriber, not even additively. Everything a workspace adds arrives as an **extension** — a workspace-local Blueprint bound one-to-one to the base row, or one-to-many child collections (a maintenance log under an adopted asset register), or structures composed from the same mechanisms. A one-to-one extension is a declared cardinality on the existing child mechanism (the BP-5 family), not a fourth composition kind, and it renders transparently: in the grid, forms and detail views its fields present as additional columns of the register, so to the user it feels like adding columns while the model records exactly what happened. Property overrides on upstream fields remain BP-19 overlay records under the tighten-only rule, because a restriction is not a schema addition.

Why locked-base wins over additive overlays, stated once: collisions become impossible by construction — an upstream version can never invalidate a local addition, so BP-19's conflict-notification branch disappears for additions; the analytics replica (RP-7) stays one schema per catalog entry plus cleanly separate extension tables, instead of N merged variants; and permissions need no new machinery — an extension is a child under PM-3's parent ceiling, so a subscriber can never use one to widen access to the base row, while extension fields carry their own rules within that ceiling. The fleet pilot is the proving case (`specs/pilots/paper-catalog.md` F1): fleet adopts the asset template and extends with vehicle fields (one-to-one) and a maintenance log (one-to-many) — no fork, no upstream change, and the former open question about overlay child collections dissolves.
...
4. *(Resolved — see decisions log: BP-28 workspace extensions.)*
...
- **The locked base and workspace extensions** (BP-28, owner decision, closing former open question 4): a subscribed organizational Blueprint is never modified by a subscriber; all additions are one-to-one or one-to-many extension tables rendered transparently as columns and child grids. Supersedes BP-19's added-fields overlay kind; tighten-only property overrides survive. The fleet pilot (asset + vehicle fields + maintenance log) adopts without forking, which the previous position could not offer.
