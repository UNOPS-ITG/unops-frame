...
**PM-14 (P2).** Steward simulation and decision trace. A principal holding `manage` on a Blueprint may evaluate its compiled rule set as any named principal, against a named row or a synthetic attribute set, and receive the full decision trace as machine-readable data: the rules that matched, the most specific deny where one fired, the unioned allow scope, the parent ceiling's effect (PM-3), and the per-field outcome per sensitivity band and field set. The trace is produced by the PM-4a evaluator itself — the compiler already computes rule specificity to choose which deny to report and then discards the rest of its reasoning; this requirement retains that reasoning behind a flag — never by a second component reasoning about rules, which would be the second implementation PM-4 forbids, arriving as admin tooling. Simulation is read-only, returns outcomes and rule identities rather than field values, and records an access-class audit event (PM-7) naming both the steward and the simulated principal, because "what can X see" is itself sensitive information. A split evaluation path cannot offer this — the reference implementation's list trimming happens inside a hand-written SQL fragment its permission trace cannot see — so the capability is only possible under the PM-4 single-evaluator invariant, and it is what turns BP-17's permission-gap review and PM-11's "who has what, via which rule" from archaeology into a query. PM-5a renders the deciding rule to end users; PM-14 is the steward-grade completion of the same principle.
...
- **Steward simulation and the full decision trace are specified** (PM-14), after verifying from the Frappe source that a dual-path evaluator can never produce one — the single-evaluator invariant's payoff made visible.
